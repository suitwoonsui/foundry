...
export function PathCard({ path }: { path: ContributionPath }) {
  return (
    <article
      className={`group flex flex-col rounded-2xl border border-white/10 bg-gradient-to-br p-5 ring-1 ring-inset transition-colors hover:border-white/20 ${path.accentClass}`}
    >
      ...
      <p className="mt-4 text-xs text-zinc-500">{path.levels.length} recognition levels · earned, not bought</p>
      <Link
